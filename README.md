# ⚡ RezTerm v3.0.2 (Cyberpunk Network Engineer Toolkit & Forensics Suite)
...
* **Motor de Eficiência Adaptativa (`v3.0.2`)**: Deteção autônoma de processadores modestos ou notebooks (`<= 4` núcleos/threads como i5-6300U, i3, Celeron). Ativação de **WMI Orientado a Eventos** (reduzindo varreduras redundantes a perto de 0% de CPU em repouso) e **Cota de Paralelismo Inteligente (`ProcessorCount - 1`)**, reservando sempre um núcleo dedicado ao Windows para que o mouse e a janela permaneçam 100% fluidos. Pop-up interativo no boot e botão de alternância imediata no menu lateral!
