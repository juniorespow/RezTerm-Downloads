# ⚡ RezTerm v3.0.3 (Cyberpunk Network Engineer Toolkit & Forensics Suite)
...
Desenvolvido na marra por **Reginaldo Junior**, um Engenheiro de Redes raiz e cansado de ver empresas e analistas pagando fortunas de licenças anuais (R$ 30.000,00+ por ano) por softwares fechados, lentos, cheios de burocracia e paywalls. O **RezTerm** nasceu da revolta da galera Nerd de tecnologia: uma suíte "root/cyberpunk" de diagnóstico e monitoramento, rodando direto no metal do Windows (.NET 10 WPF / ReadyToRun Native) com uma interface ultramoderna Blazor Hybrid em estética **Cyberpunk Dark Neon**, **Motor de Eficiência Adaptativa (`v3.0.3`)**, **Hotfix GPU Universal (`v3.0.2`)**, **Inteligência Integral de Conectividade & Anti-Oscilação (`v3.0.0`)**, **Triagem Ethernet L2 Customizável (`v3.0.0`)**, **Correção do Travamento Cisco Exit Lock no SSH (`v2.2.9`)**, **Responsividade Universal (`v3.0.0`)**, **Inteligência RF MESH / Repetidores (`v2.2.6`)** e o revolucionário **Motor Inteligente Anti-CoPP Rate-Limit 3 Fases**.

</div>

---

## 🌌 1. Identidade Visual Cyberpunk Dark Neon & Manifesto Nerd
O **RezTerm** foi projetado para longas jornadas de operação em redes e troubleshoot rápido em ambientes críticos (Datacenters, provedores e salas de controle NOC/SOC):
...
* **Motor de Eficiência Adaptativa (`v3.0.3`)**: Deteção autônoma de processadores modestos ou notebooks (`<= 4` núcleos/threads como i5-6300U, i3, Celeron). Ativação instantânea de **WMI Orientado a Eventos**, **Zero-CPU Boot Throttling** (pausando varreduras pesadas durante a tela de onboarding para impedir travamento do WebView2) e **Cota de Paralelismo Inteligente (`ProcessorCount - 1`)**, reservando sempre um núcleo dedicado ao Windows para que o mouse e a janela permaneçam 100% fluidos. Pop-up interativo no boot com a frase *"Aplicando Modo Eficiência"* e botão de alternância imediata no menu lateral!
...
* **Inteligência RF MESH & Zero Falsos Positivos (`v2.2.6`)**: Algoritmos de engenharia wireless CWNA que distinguem perfeitamente nós de redes **MESH Wi-Fi (ex: TP-Link Deco)** e repetidores legítimos de verdadeiros interferidores externos. Eliminação de falsos positivos no cálculo de Co-Canal (CCI) e Adjacente (ACI).
